...
const StudentFeeList = () => {
  ...
  const fetchFees = async () => {
    try {
      ...
      setLoading(false);
      console.log('Fees data refreshed:', response.data.length, 'records');
    } catch (err) {
  ...
  // Add this new useEffect to detect URL changes and refresh parameter
  useEffect(() => {
  ...
  // Add this useEffect to also listen for navigation state changes
  useEffect(() => {
    if (location.state && location.state.refresh) {
      console.log('Navigation state refresh detected, reloading data...');
      fetchFees();
    }
  }, [location.state]);
  ...
  const generateFeeChallan = (fee) => {
    try {
      ...
      const feeItems = [];
      if (parseFloat(fee.pending) > 0) feeItems.push(['Previous Balance', `${parseFloat(fee.pending).toFixed(2)}`]);
      ...
      const createChallanCopy = (column, copyTitle) => {
        ...
        if (fee.description && fee.description !== "na" && fee.description.trim() !== "") {
          doc.setFontSize(7);
          doc.setFont('helvetica', 'normal');
        ...
        doc.setFontSize(8);
        doc.setFont('helvetica', 'normal');
        doc.line(xStart + 5, tableY, xStart + 35, tableY);
        doc.line(xStart + colWidth - 40, tableY, xStart + colWidth - 5, tableY);
  ...
  // Calculate total fees paid and pending - updated for new model structure
  const calculateStats = () => {
    const totalFees = filteredFees.reduce((sum, fee) => sum + parseFloat(fee.total_fee), 0);
    const totalPaid = filteredFees.reduce((sum, fee) => sum + parseFloat(fee.amount_paid), 0);
    const totalBalance = filteredFees.reduce((sum, fee) => sum + parseFloat(fee.balance), 0);
  ...
  const { totalFees, totalPaid, totalBalance } = calculateStats();
  ...
  return (
    <div className="container-fluid mt-4">
      ...
      {/* Fee summary statistics - Updated to reflect new model */}
        ...
        <div className="col-md-4">
          <div className="card bg-success text-white">
            <div className="card-body">
              ...
              <p className="card-text fw-bold">Rs.{totalPaid.toFixed(2)}</p>
            </div>
          </div>
        </div>
        <div className="col-md-4">
          <div className="card bg-warning">
            <div className="card-body">
              ...
              <p className="card-text fw-bold">Rs.{totalBalance.toFixed(2)}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="table table-striped table-hover">
            ...
            <tbody>
              {filteredFees.map((fee) => (
                <tr key={fee.id} className={parseFloat(fee.balance) > 0 ? 'table-warning' : ''}>
                  <td>
                    <div className="form-check">
                      <input 
                        className="form-check-input" 
                  ...
                  <td>
                    <span className={parseFloat(fee.pending) > 0 ? 'text-warning fw-bold' : ''}>
                      {parseFloat(fee.pending).toFixed(2)}
                    </span>
                  </td>
                  ...
                  <td className="fw-bold">{parseFloat(fee.total_fee).toFixed(2)}</td>
                  <td className="text-success fw-bold">{parseFloat(fee.amount_paid).toFixed(2)}</td>
                  <td>
                    <span className={parseFloat(fee.balance) > 0 ? 'text-danger fw-bold' : 'text-success'}>
